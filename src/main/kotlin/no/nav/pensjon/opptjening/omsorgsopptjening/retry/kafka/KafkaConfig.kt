...
class KafkaConfig(@Value("\${kafka.brokers}") private val aivenBootstrapServers: String) {
    ...
    @Bean
    fun omsorgsArbeidKafkaListenerContainerFactory(kafkaSecurityConfig: KafkaSecurityConfig): ConcurrentKafkaListenerContainerFactory<String, String>? =
    ...
    private fun consumerConfig() = mapOf(
        ConsumerConfig.CLIENT_ID_CONFIG to "omsorgsopptjening-retry",
    ...
    private fun omsorgsopptjeningProducerConfig() = mapOf(
        ProducerConfig.CLIENT_ID_CONFIG to "omsorgsopptjening-retry",
